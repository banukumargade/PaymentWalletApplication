...
public class Client {
	...
	public static void main(String[] args) {
		...
		do {
			...
			System.err.println("              ************************welcome to paymentwallet application********************     ");
	...
	private static void createAccount() {
		...
		try {
			...
			System.out.println("enter aadhar your aadhar number: \n");
			long aadharNum = Long.parseLong(br.readLine());
			System.out.println("Enter Username(Atleast 4 charcters): \n");
			String userName = br.readLine();
			System.out
					.println("Enter password(should contain atleast 8 charcters(including one special charcter)) :\n");
			...
			if (isValidEmail && isValidGender && isValidPassword && isValidPhoneNumber && isValidAge
					&& isValidUserName) {
				int a = service.createAccount(details);

				if (a == 1) {
					System.err.println("Your account successfully created with account number:" + accNo);
				} else {
					try {
				...
				System.err.println("Account creation is failed due to:");
		...
		try {
			...
			if (service.testLogin(userName, password)) {
				do {
					...
					case (6):
						service.close();
						System.err.println("Logging you out and returning to main menu...........");
						break;
